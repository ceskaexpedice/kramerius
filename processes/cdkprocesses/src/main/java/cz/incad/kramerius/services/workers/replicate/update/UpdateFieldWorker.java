...
public class UpdateFieldWorker extends AbstractReplicateWorker {
    ...
    public void run() {
        try {
            ...
            for (int i=0;i<batches;i++) {
                ...
                try {
                    ...
                    if (!pidsToReplicate.getAlreadyIndexed().isEmpty()) {
                        ...
                        for (int j = 0,ll=docs.size(); j < ll; j++) {
                            ...
                            if (updateFieldElm != null && idElm != null) {
                                if (updateFieldElm.getNodeName().equals("arr")) {
                                	...
                                	List<String> collected = elements.stream().map(Element::getTextContent).collect(Collectors.toList());
                                	if (idElm != null) {
                                		map.put(idElm.getTextContent(), Pair.of(collected, idElm.getTextContent()));
                                	}
                                } else {
                            		map.put(idElm.getTextContent(), Pair.of(Arrays.asList(updateFieldElm.getTextContent()), idElm.getTextContent()));
                                }
                            } else {
                        ...
                        ReplicateFinisher.UPDATED.addAndGet(XMLUtils.getElements(destBatch.getDocumentElement()).size());
            ...
            LOGGER.info(String.format("Worker finished; All work for workers: %d; work in batches: %d; indexed: %d; updated %d, compositeIderror %d, skipped %d" ,  ReplicateFinisher.WORKERS.get(), ReplicateFinisher.BATCHES.get(), ReplicateFinisher.NEWINDEXED.get(), ReplicateFinisher.UPDATED.get(),ReplicateFinisher.NOT_INDEXED_COMPOSITEID.get(), ReplicateFinisher.NOT_INDEXED_SKIPPED.get()));
    ...
    protected Element fetchFields(List<String> subpids) {
        try {
			...
			String query =  "?q="+this.idIdentifier+":(" + URLEncoder.encode(reduce, "UTF-8") + ")&fl=" + URLEncoder.encode(this.fieldList, "UTF-8")+"&wt=xml&rows="+subpids.size();
