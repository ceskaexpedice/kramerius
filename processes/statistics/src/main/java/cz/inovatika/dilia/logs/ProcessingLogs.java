...
public class ProcessingLogs {
    ...
    public static final Logger LOGGER = Logger.getLogger(ProcessingLogs.class.getName());

    private final String apiBaseUrl;
    private final Path inputDir;
    private final Path outputDir;

    public ProcessingLogs(String apiBaseUrl, String inputDir, String outputDir) {
    ...
    public static void main(String[] args) {
        if (args.length < 3) {
            System.out.println("Použití: java ProcessLogs <api_url> <input_dir> <output_dir>");
            System.out.println("Příklad api_url: https://kramerius.vkol.cz/search/api/client/v7.0/search?q=");
            return;
        }
        try {
            ProcessingLogs processor = new ProcessingLogs(args[0], args[1], args[2]);
            processor.run();
        } catch (IOException e) {
