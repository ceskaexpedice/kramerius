...
public class RemoveLicenseStarter {

    public static final Logger LOGGER = Logger.getLogger(RemoveLicenseStarter.class.getName());

    @ProcessMethod
    public static void removeLicenseMain(
            @ParameterName("license") @IsRequired String license,
            @ParameterName("pid") @IsRequired String target
    ) {

        LOGGER.info("--- Starting method: removeLicenseMain ---");
        LOGGER.info("Parameter 'license': " + (license == null ? "N/A" : license));
        LOGGER.info("Parameter 'target': " + (target == null ? "N/A" : target));
        LOGGER.info("-------------------------------------");

        SetLicenseProcess.setLicenseMain(SetLicenseProcess.Action.REMOVE.name(),license,target );
