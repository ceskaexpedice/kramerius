...
public class SpecialNeedsEBookProcess {
    ...
    public static void generate(
        ...
        String serviceApiBaseUrl = KConfiguration.getInstance().getConfiguration().getString(GENERATE_EPUB_SERVICE_API_BASE_URL);
        LOGGER.info("serviceApiBaseUrl: " + serviceApiBaseUrl);

        //1. schedule remote export Job
        JSONObject job = scheduleRemoteJob(serviceApiBaseUrl, pid);

        //2. wait for remote export Job to finish
        while (!job.getString("state").equals("completed")) {
            try {
                job = checkRemoteJob(serviceApiBaseUrl, job.getString("job_id"));
                switch (job.getString("state")) {
                    case "pending": {
                        LOGGER.info("Waiting for Epub export to start...");
                    }
                    case "completed": {
                        LOGGER.info("Epub export has been finished successfully.");
                    }
                    default: {
                        LOGGER.info("Epub export state: " + job.getString("state"));
                    }
                    //TODO: errors
                    //TODO: progress
                }
        ...
        String url = serviceApiBaseUrl + job.getString("download_url");
        String filename = job.getString("filename");

        LOGGER.info("Filename: " + filename);
        LOGGER.info("Download url: " + url);
        notifyUsersWithEmail(pid, filename, url, email);
    }

    private static JSONObject checkRemoteJob(String exportApiBaseUrl, String jobId) {
        ...
        if (random.nextBoolean()) {//success
            return new JSONObject()
                    .put("job_id", jobId)
                    .put("state", "completed")
                    .put("filename", "Dášeňka,_čili,_Život_štěněte-algorithmic.epub")
                    .put("download_url", "/exports/bdab97866cb04865b91be733d4670ad3/download");
        } else { //error
            return new JSONObject()
                    .put("job_id", jobId)
                    .put("state", "error")
                    .put("error_message", "An error occurred during Epub export");
        }
    }

    private static JSONObject scheduleRemoteJob(String exportApiBaseUrl, String pid) {
        //TODO: implement properly
        return new JSONObject().put("state", "pending").put("job_id", "bdab97866cb04865b91be733d4670ad3");
    }
