...
public class CDKVirtualCollectionsMockTest {
    ...
    @Test
    public void testGetVirtualcollection()
        ...
        CDKVirtualCollectionsGetImpl.SimpleCollectionsCache cache = createMockBuilder(
                CDKVirtualCollectionsGetImpl.SimpleCollectionsCache.class).withConstructor()
                        .addMockedMethod("virtualCollectionsFromPoint").createMock();

        CDKResourcesFilter fi = EasyMock.createMock(CDKResourcesFilter.class);
        CDKVirtualCollectionsGetImpl get = createMockBuilder(CDKVirtualCollectionsGetImpl.class).withConstructor(cache)
                .addMockedMethod("createResourceFilter").addMockedMethod("virtualCollectionsFromPoint").createMock();
        ...
        EasyMock.expect(fi.getResources()).andReturn(
                Arrays.asList("vc:c4bb27af-3a51-4ac2-95c7-fd393b489e26", "vc:44679769-b5bb-4ac7-ad27-a0c44698c2ea"))
                .anyTimes();
        ...
        FedoraAccessImpl fedora = createMockBuilder(FedoraAccessImpl.class)
                .withConstructor(KConfiguration.getInstance(), sa).addMockedMethod("getDC").createMock();

        EasyMock.expect(fedora.getDC("vc:44679769-b5bb-4ac7-ad27-a0c44698c2ea"))
                .andReturn(XMLUtils.parseDocument(new StringReader(mzkDCSource), true)).anyTimes();
        EasyMock.expect(fedora.getDC("vc:c4bb27af-3a51-4ac2-95c7-fd393b489e26"))
                .andReturn(XMLUtils.parseDocument(new StringReader(knavDCSource), true)).anyTimes();

        EasyMock.expect(cache.virtualCollectionsFromPoint("http://kramerius.mzk.cz/search/api/v5.0/vc"))
                .andReturn(new JSONArray(mzkSource)).times(1);
        EasyMock.expect(cache.virtualCollectionsFromPoint("https://kramerius.lib.cas.cz/search/api/v5.0/vc"))
                .andReturn(new JSONArray(knavSource)).times(1);

        replay(fi, get, fedora, cache);
        get.setFedoraAccess(fedora);

        List<Collection> virtualCollections = get.virtualCollections();
        Assert.assertTrue(virtualCollections.size() == 79);

        for (Collection vc : virtualCollections) {
            Assert.assertNotNull(vc.getPid());
            Assert.assertTrue(vc.getPid().startsWith("vc:"));
        }

        // second call must be from cache
        List<Collection> anotherCall = get.virtualCollections();
        Assert.assertNotNull(anotherCall);
        Assert.assertTrue(anotherCall.size() == 79);

        List<Collection> mzkCols = get.virtualCollectionsFromResource("vc:44679769-b5bb-4ac7-ad27-a0c44698c2ea");
