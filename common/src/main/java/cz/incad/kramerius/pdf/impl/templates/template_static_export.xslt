...
<xsl:stylesheet version="1.0"
    ...
    <xsl:template match="/">
        ...
        <xsl:call-template name="new-line"/>{line} {font}(size=14)
...
PID:<xsl:text> </xsl:text> <xsl:value-of select="$pid"/>{font}(size=14)
        <!-- rendrovani autoru -->      
...
SICI:<xsl:text> </xsl:text> <xsl:value-of select="text()"/>{font}(size=14)
            </xsl:if>
            <xsl:if test="@type='urn' and text()!=''"> 
URN:<xsl:text>  </xsl:text> <xsl:value-of select="text()"/>{font}(size=14)
            </xsl:if>
            <xsl:if test="@type='issn' and text()!=''"> 
ISSN:<xsl:text> </xsl:text> <xsl:value-of select="text()"/>{font}(size=14)
            </xsl:if>
            <xsl:if test="@type='isbn' and text()!=''"> 
ISBN:<xsl:text> </xsl:text> <xsl:value-of select="text()"/>{font}(size=14)
            </xsl:if>
    </xsl:template> 
...
<xsl:value-of select="$bundle/value[@key='pdf.title']"/><xsl:text>: </xsl:text> <xsl:value-of select="mods:title/text()"/>{font}(size=14)
        <xsl:if test="mods:subTitle/text()!=''">
        <xsl:call-template name="new-line"/>
<xsl:value-of select="$bundle/value[@key='pdf.subtitle']"/><xsl:text>:  </xsl:text> <xsl:value-of select="mods:subTitle/text()"/>{font}(size=14)
        </xsl:if>

        <xsl:if test="mods:partName/text()!=''">
        <xsl:call-template name="new-line"/>
<xsl:value-of select="$bundle/value[@key='pdf.parttitle']"/><xsl:text>: </xsl:text> <xsl:value-of select="mods:partName/text()"/>{font}(size=14)
        </xsl:if>
    </xsl:template>
