...
public class CDKFormat implements ReplicationFormat {
	...
	public static final String[] DATA_STREAMS = {
			FedoraUtils.IMG_FULL_STREAM, /*FedoraUtils.IMG_THUMB_STREAM,*/
				...
				List<Element> datastreamsElements = XMLUtils.getElements(
						docElement, new XMLUtils.ElementsFilter() {
							...
							public boolean acceptElement(Element elm) {
								...
								boolean idContains = Arrays.asList(
										DATA_STREAMS).contains(idName);
	...
	private URL makeURL(Document doc, Element found, String idAttr) throws MalformedURLException {
		HttpServletRequest req = this.requestProvider.get();
		String pid = doc.getDocumentElement().getAttribute("PID");

		String imgServ =  ApplicationURL.applicationURL(req)+"/img?pid="+pid+"&stream="+idAttr+"&action=GETRAW";
		return new URL(imgServ);
	}
	...
	private void changeDataStream(Document document, Element datastream,
		...
		try {
            ...
            version.removeChild(XMLUtils.findElement( version,"binaryContent",version.getNamespaceURI()));
