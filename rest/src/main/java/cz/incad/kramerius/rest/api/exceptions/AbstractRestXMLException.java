...
public class AbstractRestXMLException extends WebApplicationException{
    ...
    public AbstractRestXMLException(String message, int status) {
        super(Response.status(status).entity(ExceptionJSONObjectUtils.fromMessage(message, status).toString()).type(MediaType.APPLICATION_XML).build());
    }
    
    public AbstractRestXMLException(String message,Exception ex, int status) {
        super(Response.status(status).entity(ExceptionJSONObjectUtils.fromMessage(message, status,ex).toString()).type(MediaType.APPLICATION_XML).build());
    }
