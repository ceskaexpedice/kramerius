...
public class V5RedirectHandler extends ProxyHandler{
    ...
    public V5RedirectHandler(Client client, SolrAccess solrAccess, String source, String pid) {
	...
	public String mods() throws ProxyHandlerException {
        String baseurl = super.baseUrl();
        String url = baseurl + (baseurl.endsWith("/") ? "" : "/") + "api/v5.0/item/" + this.pid + "/streams/BIBLIO_MODS";
        return url;
	}
	...
	public Response providedByLicenses() throws ProxyHandlerException {
        ...
        responseJson.put("licenses", new JSONArray());
	...
	public Response info() throws ProxyHandlerException {
    	try {
			...
			json.put("providedByLicenses", new JSONArray());
    ...
    private JSONObject extractImageSourceInfo(JSONObject info, JSONObject streams, JSONObject basicDoc) throws IOException, RepositoryException, LexerException {
    	JSONObject json = new JSONObject();
    ...
    protected JSONObject extractStructureChildren(JSONObject info, JSONObject basicSolrDoc, List<List<Pair<String,String>>> paths) throws ProxyHandlerException{
    ...
    protected JSONObject extractStructureInfo(String source, JSONObject info, JSONObject basicSolrDoc) throws ProxyHandlerException {
		...
		paths.forEach(path-> {
			if (path.size() > 1) {
				...
				//String collect = path.stream().map(Pair::getRight).collect(Collectors.joining("/"));
	...
	private String retrieveStreams() {
		String baseurl = super.baseUrl();
	...
	private JSONObject retrieveBasicDoc(String pid) throws ProxyHandlerException {
		try {
			String baseurl = super.baseUrl();
			String query = URLEncoder.encode("PID:\"" + pid + "\"", "UTF-8");
			String url = baseurl + (baseurl.endsWith("/") ? "" : "/") + "api/v5.0/search?q="+query+"&wt=json&fl="+URLEncoder.encode("img_full_mime fedora.model pid_path parent_pid", "UTF-8");
			WebResource r = this.client.resource(url);
			String t = r.accept(MediaType.APPLICATION_JSON).get(String.class);
	...
	private Pair<JSONArray,JSONArray> retrieveChildren(JSONObject info, JSONObject basicSolrDoc , List<List<Pair<String,String>>> paths) throws UnsupportedEncodingException {
		int maxRows = 4000;
		String baseUrl = super.baseUrl();
		String url = "";
		...
		if (model.equals(OwnRelationsMapping.article.name())) {
			...
			String encoded = URLEncoder.encode("pid_path:" + path.replace(":", "\\:"), "UTF-8")+"*"+URLEncoder.encode(" AND NOT PID:" + pid.replace(":", "\\:"), "UTF-8");
			url = baseUrl + (baseUrl.endsWith("/") ? "" : "/") + "api/v5.0/search?q=*&fq="+encoded+"&wt=json&rows="+maxRows+"&fl="+URLEncoder.encode("img_full_mime model_path fedora.model pid_path PID rels_ext_index", "UTF-8");
			...
			String query = URLEncoder.encode("parent_pid:" + pid.replace(":", "\\:")+" AND NOT PID:"+pid.replace(":", "\\:"), "UTF-8"); 
			url = baseUrl + (baseUrl.endsWith("/") ? "" : "/") + "api/v5.0/search?q=*&fq="+query+"&wt=json&rows="+maxRows+"&fl="+URLEncoder.encode("img_full_mime model_path fedora.model pid_path PID rels_ext_index", "UTF-8");
