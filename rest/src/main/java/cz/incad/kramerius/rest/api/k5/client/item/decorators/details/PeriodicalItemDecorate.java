            ...
            try {
                ...
                if (doc != null) {
                    ...
                    if (!array.isEmpty()) {
                        ...
                        if (details.length > 1) {
                            detailsJSONObject.put("issueNumber", ItemResourceUtils.preventAutomaticConversion(details[1]));
                        }
                        if (details.length > 2) {
                            detailsJSONObject.put("date", details[2]);
                        }
                        if (details.length > 3) {
                            detailsJSONObject.put("partNumber", ItemResourceUtils.preventAutomaticConversion(details[3]));
                        }
                        boolean moreThenZero = detailsJSONObject.keys().hasNext();
                        if (moreThenZero) {
