...
public class ReplicationsResource {
    ...
    boolean checkPermission(String pid) throws IOException {
        ...
        for (ObjectPidsPath pth : paths) {
            if (this.isActionAllowed.isActionAllowed(SecuredActions.EXPORT_K4_REPLICATIONS.getFormalName(), pid, null, pth).flag()) return true;
        }
        if (paths.length == 0) {
            ObjectPidsPath path = new ObjectPidsPath(SpecialObjects.REPOSITORY.getPid());
            if (this.isActionAllowed.isActionAllowed(SecuredActions.EXPORT_K4_REPLICATIONS.getFormalName(), SpecialObjects.REPOSITORY.getPid(), null, path).flag()) return true;
        }
        return false;
    }
