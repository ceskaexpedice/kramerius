...
var ProgressBar = (function ProgressBarClosure() {
  ...
  function ProgressBar(id, opts) {
    ...
    //this.div = document.querySelector(id + ' .spinner');
  ...
  PDFFindBar.prototype = {
    ...
    findEvent: function PDFFindBar_dispatchEvent(type, findPrev, settingsObject) {
      ...
      event.initCustomEvent('find' + type, true, true, {
        ...
        caseSensitive: settingsObject.caseSensitive === true,
        highlightAll: settingsObject.highlightAll === true,
        findPrevious: findPrev
      });
      return window.dispatchEvent(event);
    },
...
window.addEventListener("message", function(e) {
  var parametrized = e.data.split("=");
  if (parametrized[0] === "query") {
    if (parametrized.length > 1) {
      var q = parametrized[1];
      findInDoc(q);
    }
  } else if (parametrized[0] === "zoomIn") {
    PDFViewerApplication.zoomIn();
  } else if (parametrized[0] === "zoomOut") {
    PDFViewerApplication.zoomOut();
  }
...
document.addEventListener("pagesloaded", function(e) {
  var queryString = document.location.search.substring(1);
  var params = parseQueryString(queryString);
  var q = 'query' in params ? params.query : null;
  if (q) {
    findInDoc(q);
  }

  document.getElementById("k5loadingBar").style.display = "none";
