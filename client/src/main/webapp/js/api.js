...
ClientAPIDev.prototype = {
	...
	askForPopular: function(whenready) {
		...
		if (whenready) whenready.apply(null, [data]);
		this.application.eventsHandler.trigger("api/feed/mostdesirable",data);
	    },this));
	},
    ...
    */
    askForCollections: function(whenready) {
        $.getJSON("api/vc",_.bind(function(data) {
                ...
                for(var i=0; i<data.length; i++){
                    var pid = data[i].pid;
                    collections[pid] = {"cs": data[i].descs.cs, "en": data[i].descs.en};
                }
	...
	askForSolr:function(query, whenready) {
	    //"?fl=dc.creator,dc.title,PID,dostupnost&fq=" + URLEncoder.encode(fq, "UTF-8") + "&q=rok:" + year + "&start=" + offset + "&rows=" + rows;
	...
	askForItem: function(pid, whenready) {
		...
		this.ctx["item"][pid]=data; 
                ...
                $.getJSON("api/item/"+pid+"/streams", _.bind(function(sdata) {
        		this.ctx["item"][pid]["streams"]=sdata; 
                        this.ctx.callhistory.push(pid);
        		if (whenready) whenready.apply(null, [data]);
        		this.application.eventsHandler.trigger("api/item/"+pid,data);
                },this));
	...
	/** 
		...
		if (whenready) whenready.apply(null, [data]);
		this.application.eventsHandler.trigger("api/item/"+pid+"/streams",data);
	    },this));
        },	
	...
	askForItemSiblings: function(pid, whenready) {
		...
		if (whenready) whenready.apply(null, [data]);
		this.application.eventsHandler.trigger("api/item/"+pid+"/siblings",data);
	    },this));
	},
	...
	askForItemChildren: function(pid, whenready) {
		...
		if (whenready) whenready.apply(null, [data]);
		this.application.eventsHandler.trigger("api/item/"+pid+"/children",data);
	    },this));
	},
	...
	askForCache: function(filename, whenready, onerror) {
	    ...
	    $.get("cache?action=get&f=" + filename, _.bind(function(data) {
		if (!this.isKeyReady("cache/"+filename)) {
                        this.ctx["cache"][filename]={}; 
                }
		this.ctx["cache"][filename]=data; 
		if (whenready) whenready.apply(null, [data]);
		this.application.eventsHandler.trigger("api/cache/"+filename,data);
	    },this)).error(function(data){
                console.log("error asking for cache: " + data);
		if (onerror) onerror.apply(null, [data]);
            });
	},
	...
	saveToCache: function(filename, content, whenready) {
	    $.post("cache", {action:"save", f:filename, c:content}, _.bind(function(data) {
                if (!this.isKeyReady("cache")) {
                        this.ctx["cache"]={};
		}
		if (!this.isKeyReady("cache/"+filename)) {
	...
	vctranslatable: function(key) {
        ...
        /**
         * Search given pid and find first component to display
         * ommit every 
         */
	searchItemAndExploreChildren: function(pid, whenready) {
		...
		this.ctx["item"][pid]['children']=data; 
        		...
        		if (whenready) whenready.apply(null, [pid]);
                }
            },this));    
        },
