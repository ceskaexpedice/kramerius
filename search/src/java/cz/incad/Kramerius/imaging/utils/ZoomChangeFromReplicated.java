...
public class ZoomChangeFromReplicated {
	...
	private static String replicatedFrom(Document relsExt) {
		Element descElement = XMLUtils.findElement(relsExt.getDocumentElement(), "Description",FedoraNamespaces.RDF_NAMESPACE_URI);
		List<Element> delems = XMLUtils.getElements(descElement);
		for (Element del : delems) {
			if (del.getNamespaceURI() != null) {
				if (del.getNamespaceURI().equals(FedoraNamespaces.KRAMERIUS_URI) && del.getLocalName().equals("replicatedFrom")) {
