...
<xsl:stylesheet  version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" >
    ...
    <xsl:template name="tabs">
        <ul>
            <xsl:for-each select="//doc[generate-id(.) = generate-id(key('keyModel', str[@name='fedora.model'])[1])]">
                ...
                <li class="model">
                    <xsl:attribute name="id"><xsl:value-of select="$path" />-<xsl:value-of select="$model" />_<xsl:value-of select="$pid" /></xsl:attribute>
    ...
    <xsl:template name="rels">
        ...
        <xsl:for-each select="//doc[str[@name='fedora.model']=$fmodel]" >
            <li>
                <xsl:attribute name="id"><xsl:value-of select="$path" />-<xsl:value-of select="$fmodel" />_<xsl:value-of select="./str[@name='PID']" /></xsl:attribute>
