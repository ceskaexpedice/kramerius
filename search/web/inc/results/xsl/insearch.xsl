<xsl:stylesheet  version="1.0"
    ...
    <xsl:template name="pagination">
        ...
        <div class="numDocs"><xsl:value-of select="$numDocs" />&#160;<xsl:value-of select="$numDocsStr" />
    ...
    <xsl:template name="doc">
        ...
        <td class="resultThumb" valign="top">
            <a>
                <xsl:attribute name="href">javascript:loadItemMenu('pid=<xsl:value-of select="$pid"/>&amp;pid_path=<xsl:value-of select="./str[@name='pid_path']"/>&amp;path=<xsl:value-of select="./str[@name='path']"/>');
                </xsl:attribute>
