...
$(document).ready(function(){
    ...
    var plot1 = $.jqplot('pidschart', data, options);

    $('#pidschart').bind('jqplotDataUnhighlight', function()  {
        $("#pidscharttooltip").hide();
    });
    
    $('#pidschart').bind('jqplotDataHighlight', 
            function (ev, seriesIndex, pointIndex, data ) {
                var mouseX = ev.pageX; //these are going to be how jquery knows where to put the div that will be our tooltip
                var mouseY = ev.pageY;
                // max left position
                var max = $('#pidschart').width() - 200;

                var calculatedY=mouseY - $('#pidschart').offset().top-70 ;
                var calculatedX = mouseX-$('#pidschart').offset().left+70;
                calculatedX = Math.min(calculatedX,max);
                
                $("#pidscharttooltip").show();
                $("#pidscharttooltip").css("position", "absolute").css("top",""+calculatedY+"px").css("left",""+calculatedX+"px");
                
                $("#pidscharttooltip").html('<div style="margin:5px;"><span><i><view:msg>common.waitplease</view:msg>...</i></span></div>');
                $('#pidscharttooltip').show();
                $.get("inc/admin/_statistics_model_details.jsp?pid="+pids[seriesIndex],function(dat) {
                    $("#pidscharttooltip").html(dat);
    ...
    <div id="model_report_graph" style="padding-top: 10px; position: relative;">
        <div id="pidschart" style="width:750px; height:370px;"></div>
        <div id="pidscharttooltip" style="position:absolute;font-style: normal; display: none;" class="shadow ui-widget ui-widget-content"></div>
    </div>
