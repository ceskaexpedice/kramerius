...
<div class="usedFilter">
    :: <a title="" class="mainNav"
     ...
     title="<fmt:message key="remove_criteria"/><fmt:message key="query" />: <c:out value="${param.q}" />" />
</a></div>
</c:if>
<%-- datum --%>
<c:if test="${param.f1 != null}">
    <fmt:message key="Datum" />: <c:out value="${param.f1}" /> - <c:out value="${param.f2}" />
</c:if>
...
<c:if test="${!empty param.issn}">
    ...
    <fmt:message key="issn" />: <c:out value="${param.issn}"/>&#160;<img src="img/x.png"  border="0" 
    title="<fmt:message key="remove_criteria"/>: issn"/></a></div>
</c:if>
<c:if test="${!empty param.title}">
    <div class="usedFilter">:: 
    <a title="" class="mainNav" href="javascript:removeNavigation2('title', '<c:out value="${param.title}" />');">
    <fmt:message>Hlavní název</fmt:message>: <c:out value="${param.title}"/>&#160;<img src="img/x.png"  border="0" 
    title="<fmt:message key="remove_criteria"/>: title"/></a></div>
</c:if>
<c:if test="${!empty param.author}">
    <div class="usedFilter">:: 
    <a title="" class="mainNav" href="javascript:removeNavigation2('author', '<c:out value="${param.author}" />');">
    <fmt:message key="author" /> <c:out value="${param.author}"/>&#160;<img src="img/x.png"  border="0" 
    title="<fmt:message key="remove_criteria"/>: author"/></a></div>
</c:if>
<c:if test="${!empty param.rok}">
    <div class="usedFilter">:: 
    <a title="" class="mainNav" href="javascript:removeNavigation2('rok', '<c:out value="${param.rok}" />');">
    <fmt:message key="rok" /> <c:out value="${param.rok}"/>&#160;<img src="img/x.png"  border="0" 
    title="<fmt:message key="remove_criteria"/>: rok"/></a></div>
</c:if>
<c:if test="${!empty param.udc}">
    <div class="usedFilter">:: 
    <a title="" class="mainNav" href="javascript:removeNavigation2('udc', '<c:out value="${param.udc}" />');">
    <fmt:message key="MDT" /> <c:out value="${param.udc}"/>&#160;<img src="img/x.png"  border="0" 
    title="<fmt:message key="remove_criteria"/>: udc"/></a></div>
</c:if>
<c:if test="${!empty param.ddc}">
    <div class="usedFilter">:: 
    <a title="" class="mainNav" href="javascript:removeNavigation2('ddc', '<c:out value="${param.ddc}" />');">
    <fmt:message key="DDT" /> <c:out value="${param.ddc}"/>&#160;<img src="img/x.png"  border="0" 
    title="<fmt:message key="remove_criteria"/>: ddc"/></a></div>
</c:if>
<c:if test="${!empty param.onlyPublic}">
    ...
    <fmt:message key="Pouze veřejné dokumenty" /> <c:out value="${param.onlyPublic}"/>&#160;<img src="img/x.png"  border="0" 
    title="<fmt:message key="remove_criteria"/>: onlyPublic"/></a></div>
</c:if>
<%--
<br/>
