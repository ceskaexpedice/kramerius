...
<table width="100%">
	<thead style="border-bottom: dashed 1px;" class="result r1">
		<tr>
			<td>${processView.nameOrdering}</td>
			<td>${processView.pidOrdering}</td>
			<td>${processView.stateOrdering}</td>
			<td>${processView.dateOrdering}</td>
	...
	<tbody>
		<c:forEach var="lrProc" items="${processView.processes}" varStatus="i">
			<tr class="${(i.index mod 2 == 0) ? 'result r0': 'result r1'}">
				...
				<td>${lrProc.start}</td>
				<td>
					<a href="${lrProc.killURL}">Zastavit</a>
				</td>
			</tr>
		</c:forEach>
	</tbody>
