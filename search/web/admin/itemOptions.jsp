...
<%@page import="cz.incad.kramerius.service.ResourceBundleService"%><div class="menuOptions" id="openmenu<c:out value="${status.count}" />-<c:out value="${itemViewObject.models[status.count -1]}"/>" style="float:right;"  >
    <span class="menu_activation">
        <a href="javascript:toggleAdminOptions(<c:out value="${status.count}" />, '<c:out value="${itemViewObject.models[status.count -1]}" />');" >
            <img border="0" title="<fmt:message bundle="${lctx}">administrator.menu</fmt:message>" alt="<fmt:message bundle="${lctx}">administrator.menu</fmt:message>" src="img/menu.png" />
        </a></span>
</div>
        ...
        <table width="100%">
            <tr>
                ...
                <td width="25px" align="center">
                    <a href="javascript:hideContextMenu();" class="ui-dialog-titlebar-close ui-corner-all" role="button" unselectable="on" style="-moz-user-select: none;"><span class="ui-icon ui-icon-closethick" unselectable="on" style="-moz-user-select: none;">close</span></a>
                </td>
            </tr>
        </table>
    </div>
