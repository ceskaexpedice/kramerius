...
<%@page import="cz.incad.kramerius.service.ResourceBundleService"%><div class="menuOptions" id="openmenu<c:out value="${status.count}" />-<c:out value="${itemViewObject.models[status.count -1]}"/>" style="float:right;"  >
    <span class="menu_activation"><img title="<fmt:message bundle="${lctx}">administrator.menu</fmt:message>" src="img/menu.png" onclick="toggleAdminOptions(<c:out value="${status.count}" />, '<c:out value="${itemViewObject.models[status.count -1]}" />');" /></span>
</div>
<div style="display:none;float:right;position:absolute;z-index:7; background-color: white;" class="menuOptions" id="menu<c:out value="${status.count}" />-<c:out value="${itemViewObject.models[status.count -1]}" />" >
